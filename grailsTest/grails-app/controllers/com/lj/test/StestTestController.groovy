...
class StestTestController {
    ...
    def list(Integer max) {
        ...
        [stestTestInstanceList: StestTest.list(params), stestTestInstanceTotal: StestTest.count()]
    }

    def create() {
        [stestTestInstance: new StestTest(params)]
    }

    def save() {
        def stestTestInstance = new StestTest(params)
        if (!stestTestInstance.save(flush: true)) {
            render(view: "create", model: [stestTestInstance: stestTestInstance])
            return
        }

        flash.message = message(code: 'default.created.message', args: [message(code: 'stestTest.label', default: 'StestTest'), stestTestInstance.id])
        redirect(action: "show", id: stestTestInstance.id)
    }

    def show(Long id) {
        def stestTestInstance = StestTest.get(id)
        if (!stestTestInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'stestTest.label', default: 'StestTest'), id])
            redirect(action: "list")
            return
        }

        [stestTestInstance: stestTestInstance]
    }
    ...
    def update(Long id, Long version) {
        ...
        if (version != null) {
            if (stestTestInstance.version > version) {
                stestTestInstance.errors.rejectValue("version", "default.optimistic.locking.failure",
                        [message(code: 'stestTest.label', default: 'StestTest')] as Object[],
                        "Another user has updated this StestTest while you were editing")
                render(view: "edit", model: [stestTestInstance: stestTestInstance])
                return
            }
        }

        stestTestInstance.properties = params
        ...
        flash.message = message(code: 'default.updated.message', args: [message(code: 'stestTest.label', default: 'StestTest'), stestTestInstance.id])
        redirect(action: "show", id: stestTestInstance.id)
    }
    ...
    def delete(Long id) {
        ...
        try {
            stestTestInstance.delete(flush: true)
            flash.message = message(code: 'default.deleted.message', args: [message(code: 'stestTest.label', default: 'StestTest'), id])
            redirect(action: "list")
        }
        catch (DataIntegrityViolationException e) {
            flash.message = message(code: 'default.not.deleted.message', args: [message(code: 'stestTest.label', default: 'StestTest'), id])
    ...
    def listJson() {
        ...
        def conditions = {
            ...
            if (params.ddllTest && params.ddllTest == "true") {

                double ddllTestKey = 0d;
                try {
                    ddllTestKey = Double.parseDouble(params.ddllTestKey);
                } catch (Exception ex) {}
                if (params.ddllTestOperator == "equals") {
                    eq("ddllTest", ddllTestKey);
                } else if (params.ddllTestOperator == "gt") {
                    gt("ddllTest", ddllTestKey);
                } else if (params.ddllTestOperator == "ge") {
                    ge("ddllTest", ddllTestKey);
                } else if (params.ddllTestOperator == "lt") {
                    lt("ddllTest", ddllTestKey);
                } else if (params.ddllTestOperator == "le") {
                    le("ddllTest", ddllTestKey);
                }
        ...
        def rows = StestTest.createCriteria().list(params, conditions);
        def totalCount = StestTest.createCriteria().count(conditions);
        ...
        if (idsP) {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                try {
                    ...
                    def message = message(code: 'default.not.deleted.message', args: [message(code: 'stestTest.label', default: 'StestTest'), id])
                    def resultObj = [success: false, message: message];
                    render resultObj as JSON;
                    return
                }
            }
            def message = message(code: 'default.deleted.message', args: [message(code: 'stestTest.label', default: 'StestTest'), ids])
